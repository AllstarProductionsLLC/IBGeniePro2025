...
export const getIbGenieAssertion = webMethod(
  Permissions.SiteMember,
  async (nonce) => {
    if (typeof nonce !== "string" || !/^[a-f0-9]{48}$/.test(nonce))
      throw new Error("Invalid connection request.");
    try {
      const member = await currentMember.getMember();
      if (!member?._id) throw new Error("Sign in first.");
      const allOrders = [];
      for (let skip = 0; ; skip += 100) {
        if (skip >= 1000) throw new Error("Order review required.");
        // Keep member permissions: no elevation, browser member IDs or client Pro claims.
        const page = await orders.listCurrentMemberOrders(
          {},
          {},
          { limit: 100, skip },
        );
        if (!Array.isArray(page)) throw new Error("Order lookup unavailable.");
        allOrders.push(...page);
        if (page.length < 100) break;
      }
      const [bridge, app] = await Promise.all([
        readSecret("IBGENIE_BRIDGE_SECRET"),
        readSecret("IBGENIE_APP_ORIGIN"),
      ]);
      return {
        assertion: makeAssertion({
          memberId: member._id,
          plans: paidPlans(allOrders, member._id),
          nonce,
          secret: bridge.value,
          appOrigin: app.value,
          issuer: ISSUER,
