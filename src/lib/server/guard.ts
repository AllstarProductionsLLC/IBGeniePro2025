...
const COOKIE = "ibgenie_access";
const buckets = new Map<string, { count: number; expires: number }>();
export function accessConfigured() {
  return (
    (process.env.AI_ACCESS_CODE?.length ?? 0) >= 12 &&
...
export function sameOrigin(r: Request) {
  if (
    ...
    r.headers.get("origin") !==
      (process.env.APP_ORIGIN || new URL(r.url).origin)
  )
    throw new ApiError(403, "Open this action from your IBGenie workspace.");
}
...
export function sessionIdentity(r: Request): string | null {
  ...
  if (
    ...
    return null;
  return secureEqual(signature, sign(id + "." + expires)) ? id : null;
}
export function createAccessCookie(r: NextResponse) {
  const payload =
    randomBytes(16).toString("hex") + "." + (Date.now() + 8 * 3600000);
  r.cookies.set(COOKIE, payload + "." + sign(payload), {
    httpOnly: true,
...
export function clearAccessCookie(r: NextResponse) {
...
export async function rateLimit(key: string, limit: number, seconds: number) {
  ...
  if (url && token) {
    if (!url.startsWith("https://"))
      throw new ApiError(503, "AI access needs configuration.");
    try {
      ...
      if (!r.ok || typeof d.result !== "number") throw new Error();
      if (d.result > limit)
        throw new ApiError(
          429,
          "This workspace has reached its AI usage limit. Try again later.",
        );
    } catch (e) {
      if (e instanceof ApiError) throw e;
      throw new ApiError(503, "AI access is temporarily unavailable.");
    }
    return;
  }
  ...
  const b = buckets.get(key) || { count: 0, expires: now + seconds * 1000 };
  b.count++;
  buckets.set(key, b);
  if (b.count > limit)
...
export async function requireAI(
  r: Request,
  capability: "text" | "voice" = "text",
) {
  sameOrigin(r);
  if (!accessConfigured())
    throw new ApiError(
      503,
      "AI tools are not connected yet. You can still create resources and study without AI.",
    );
  const id = sessionIdentity(r);
  if (!id)
    throw new ApiError(401, "Enter your workspace access code to use AI.");
  if (
    !(capability === "voice"
