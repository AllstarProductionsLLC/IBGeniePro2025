...
}).refine(p => new Set(p.slides.map(s => s.id)).size === p.slides.length, "Slide IDs must be unique");
...
export const sequenceSchema = z.object({
  ...
  if (s.units.some(u => u.year > s.years)) ctx.addIssue({code:"custom", message:"A unit falls outside the selected years"});
  for (let year = 1; year <= s.years; year++) {
    const units = s.units.filter(u => u.year === year);
    if (!units.length) ctx.addIssue({code:"custom", message:"Include at least one unit in each year"});
    if (units.reduce((n,u) => n + u.weeks, 0) > 52) ctx.addIssue({code:"custom", message:"A year cannot exceed 52 teaching weeks"});
  }
});
...
export function toolsForRole(role: "teacher" | "student") {
  return role === "teacher"
    ? ["presentation", "lesson-plan", "scope-sequence", "quiz", "rubric", "exit-ticket", "flashcards", "study-guide"] as const
...
export function sequenceMarkdown(sequence: Sequence) {
  return Array.from({length: sequence.years}, (_,i) => {
    const year = i+1;
    return `## Year ${year}\n\n` + sequence.units.filter(u=>u.year===year).map(u =>
...
export function presentationMarkdown(presentation: Presentation) {
  return presentation.slides.map((s,i) => `## Slide ${i+1}: ${s.title}\n\n${s.bullets.map(b=>"- "+b).join("\n")}\n\n${s.prompt}\n\n**Speaker notes**\n${s.notes}`).join("\n\n");
}
export function feedbackMarkdown(record: AssessmentRecord) {
  ...
  r.criteria.map(c => {const def=record.criteria.find(d=>d.id===c.id);return `## ${def?.label || "Criterion"}${c.score !== null && def?.max ? ` · Suggested ${c.score}/${def.max}` : ""}\n\n${c.evidence ? `Evidence: “${c.evidence}”\n\n` : "Evidence is insufficient for a mark.\n\n"}${c.rationale}\n\nNext: ${c.nextStep}`;}).join("\n\n")+
