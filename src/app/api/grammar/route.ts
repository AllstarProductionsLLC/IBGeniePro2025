...
const schema = z.object({profile:contextSchema, work:z.string().min(10).max(12000), language:z.string().min(1).max(60), dialect:z.enum(["British English","American English","Match my text"]), consent:z.literal(true)});
export async function POST(r: Request) {
  ...
  try {
    ...
    const raw = await generateText(tutorInstructions(d.profile,d.language) +
      ...
      [{role:"user",parts:[{text:d.work}]}],true);
    return NextResponse.json(validateGrammar(JSON.parse(raw),d.work),{headers:{"Cache-Control":"no-store"}});
