...
const schema = z
  ...
  .superRefine((d, ctx) => {
    if (new Set(d.criteria.map((c) => c.id)).size !== d.criteria.length)
      ctx.addIssue({ code: "custom", message: "Duplicate criteria" });
    if (d.criteria.reduce((n, c) => n + c.descriptors.length, 0) > 16000)
      ctx.addIssue({ code: "custom", message: "Rubric is too long" });
    if (
      d.scoring &&
      (!d.criteria.length ||
        !d.rubricSource.trim() ||
        d.profile.program === "pyp")
    )
      ctx.addIssue({
        code: "custom",
        message: "Add the applicable rubric before requesting marks",
      });
  });
export async function POST(r: Request) {
  ...
  try {
    ...
    const prompt =
      tutorInstructions(d.profile, d.subject, "Feedback on my reasoning") +
      "\n" +
    ...
    try {
      ...
      throw new ApiError(
        502,
        "The feedback draft was not usable. Try a shorter selection.",
      );
    }
    const report = validateFeedback(value, d.work, d.criteria, d.scoring);
