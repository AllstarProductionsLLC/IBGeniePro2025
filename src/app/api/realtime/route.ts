...
import { hangupCall, scheduleVoiceEnd } from "@/lib/server/voice";
  ...
  try {
    ...
    const response = await fetch("https://api.openai.com/v1/realtime/calls", {
      ...
      headers: {
        ...
        "OpenAI-Safety-Identifier": lease.identity,
      },
      body: form,
    ...
    const match = response.headers
      .get("location")
      ?.match(/\/calls\/([a-zA-Z0-9_-]+)\/?$/);
    if (!match)
      throw new ApiError(
        502,
        "The voice service did not return a controllable session.",
      );
    callId = match[1];
    const sdp = await response.text();
    if (!sdp.startsWith("v=0"))
      throw new ApiError(502, "The voice connection was invalid.");
    const timer = await scheduleVoiceEnd(callId, lease.identity);
    ...
    if (callId)
      try {
        ...
        console.warn("Voice cleanup requires provider review");
      }
